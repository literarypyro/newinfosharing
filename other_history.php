...
/* ==========================================================================
   @causefilter -- Filter the register by cause/issue, including the rows that
   have no recorded cause.

   THE REASON THIS IS NOT A WHERE CLAUSE.
   Only ONE of the three states a row can be in exists in the database. A
   recorded cause is incident_report.equipt joined to other_problem, and SQL
   can filter that. But a blank row is either "Uncategorized" or carries a
   cause SUGGESTED by the description classifier, and that distinction is
   computed in pass 2 below, after the query has already run. A WHERE clause
   could offer the recorded causes and nothing else -- which would leave out
   precisely the rows this filter is most useful for.

   So the query stays as it is and the filter is applied in the render loop.
   That is not merely convenient, it is required: the classifier TRAINS on
   the rows that have a cause, so narrowing the query to one cause would
   retrain the model on that cause alone and every suggestion on the page
   would change to match whatever was filtered. The model must always see the
   whole period. Filtering after training keeps the suggestions identical to
   what the unfiltered page shows.

   Values: '' = everything, a cause name, __uncat = no cause and no
   confident suggestion, __sugg = every row carrying a suggestion.
   ========================================================================== */
$causeMap=array();
...
$ohCause = isset($_GET['cause']) ? trim((string)$_GET['cause']) : '';
/* Validated against the real list rather than trusted. An unrecognised value
   is treated as no filter -- a silently empty register with a stale cause name
   in the URL is worse than ignoring it. */
if($ohCause !== '' && $ohCause !== '__uncat' && $ohCause !== '__sugg'
   && !in_array($ohCause, array_values($causeMap), true)){
	$ohCause = '';
}
$ohCauseLabel = '';
if($ohCause === '__uncat')      $ohCauseLabel = 'Uncategorized only';
else if($ohCause === '__sugg')  $ohCauseLabel = 'Suggested causes only';
else if($ohCause !== '')        $ohCauseLabel = $ohCause;

/* @selfpage -- The two places this file names itself, resolved from the actual
   filename rather than hardcoded. Both are self-references: the filter form
   posts back here, and Clear returns here. A renamed copy -- a test build
   sitting beside the live page -- would otherwise submit INTO the live page
   and lose the filter, which makes the copy untestable in the one way that
   matters. Same pattern statistics_report_modified.php uses for $selfPage.
   Rename the file freely; rename it back and nothing needs editing. */
$ohSelf = basename(__FILE__);

/* Clear returns to this page with no cause AND no period, matching what the
   button says. equipment_history.php passes its own base the same way. */
$ohClearUrl = $ohSelf;
?>
<style type='text/css'>
<?php phFilterCss(); ?>
...
<div class="ccs-header">
	...
	<div class="sub">Incidents categorized as "Others" &mdash; <?php echo htmlspecialchars($ph['label']); ?><?php
		/* @causefilter -- Named in the heading. A register showing a fraction
		   of the period with nothing saying why reads as missing data. */
		if($ohCauseLabel !== ''){ echo ' &mdash; '.htmlspecialchars($ohCauseLabel); }
	?> &mdash; Line 3</div>
</div>
...
<div class="ccs-panel-head"><h3>Incident History</h3>
  <div class="ccs-panel-actions">
    <form method="get" action="<?php echo htmlspecialchars($ohSelf); ?>" class="ph-filters">
    <?php /* @causefilter -- Ahead of the shared period fields, matching the
             order equipment_history.php uses for its Car and Level selects.
             The two synthetic entries are separated from the real causes: they
             are questions about the RECORD rather than about the incident, and
             running them into the same list invites reading "Uncategorized" as
             a kind of incident. */ ?>
    <div class="ph-field">
      <label for="ohCauseSel">Cause/Issue</label>
      <select name="cause" id="ohCauseSel">
        <option value="">All causes</option>
        <?php foreach(array_values($causeMap) as $cnm){ ?>
        <option value="<?php echo htmlspecialchars($cnm); ?>"<?php echo $ohCause===$cnm?' selected':''; ?>><?php echo htmlspecialchars($cnm); ?></option>
        <?php } ?>
        <option value="" disabled>&mdash;&mdash;&mdash;</option>
        <option value="__sugg"<?php echo $ohCause==='__sugg'?' selected':''; ?>>Suggested only</option>
        <option value="__uncat"<?php echo $ohCause==='__uncat'?' selected':''; ?>>Uncategorized only</option>
      </select>
    </div>
    <?php phFilterFields($ph, $db, $ohClearUrl); ?>
    </form>
  </div>
</div>
...
// @causefilter -- loaded higher up now: the filter dropdown needs it, and that
// renders above this point. Left as one load, not two.
...
/* @causefilter -- Tallied across EVERY row before the filter is applied, so
   the page can say how many rows the filter is holding back. Without it a
   narrowed register is indistinguishable from a quiet period. */
$ohPeriodCounts = array();
$ohPeriodTotal  = 0;
...
foreach($allRows as $row){
	...
	/* @causefilter -- Unfiltered tallies first. */
	if(!isset($ohPeriodCounts[$cause])) $ohPeriodCounts[$cause]=0;
	$ohPeriodCounts[$cause]++;
	$ohPeriodTotal++;

	/* @causefilter -- Then the gate. Everything past this point -- the table
	   row, the monthly matrix, the Pareto totals, the hour-of-day counts and
	   the analysis context -- is built from matching rows only, so no part of
	   the page can disagree with the register above it.

	   "Suggested only" deliberately ignores WHICH cause was suggested: it is
	   a data-quality view, asking which rows are running on inference rather
	   than on a recorded category. Pick a named cause to get both its
	   recorded and its suggested rows together, which is the maintenance
	   question rather than the records question. */
	if($ohCause !== ''){
		if($ohCause === '__uncat'){       if($cause !== 'Uncategorized') continue; }
		else if($ohCause === '__sugg'){   if(!$isSuggested) continue; }
		else {                            if($cause !== $ohCause) continue; }
	}

	/* @causefilter -- The suggestion tallies moved below the gate so the
	   footnote and the analysis report what is ON SCREEN. The classifier
	   itself still trained on the full period, above. */
	if($isSuggested){
...
?>	
	<tr data-mo="<?php echo $monthKey; ?>">
		...
		*/
		?>
		<?php /* @causefilter -- A sort key, so DataTables orders on the cause
		         itself. Sorting on the rendered text put "Roof Leak
		         (suggested)" after every plain "Roof Leak" instead of beside
		         them, which is the opposite of what someone sorting this
		         column wants to see. data-order is 1.10+, data-sort is 1.9;
		         both are emitted because the page loads either build. */ ?>
		<td data-order="<?php echo htmlspecialchars($cause); ?>" data-sort="<?php echo htmlspecialchars($cause); ?>"><?php
...
/* @causefilter -- What the filter is hiding, stated under the table. The
   number is the honest counterpart to the heading: it turns "this period was
   quiet" into "this period was quiet FOR THIS CAUSE". */
if($ohCause !== ''){
	$ohShown  = 0;
	foreach($problemCounts as $ohC => $ohN){ $ohShown += (int)$ohN; }
	$ohHidden = $ohPeriodTotal - $ohShown;
	if($ohHidden > 0){
		echo '<p class="ccs-note" style="color:#7A1F1F;">Showing '.(int)$ohShown.' of '
		   . (int)$ohPeriodTotal.' incidents in this period. '.(int)$ohHidden
		   . ' with a different cause '.($ohHidden==1?'is':'are').' hidden by the Cause/Issue filter, '
		   . 'and every chart and figure below counts only the '.(int)$ohShown.' shown.</p>';
	}
}
	...
	if(count($issRows) >= 1 && $issGrand >= 1){
		$issCtx = array(
			...
			'report' => array('id'=>'other_history',
			                  /* @causefilter -- carried into the analysis so a
			                     narrowed panel cannot describe itself as the
			                     whole picture. */
			                  'title'=>'Other Recorded Incidents - by cause'
			                           .($ohCauseLabel !== '' ? ' ('.$ohCauseLabel.')' : ''),
			...
			'filters'=> ($ohCauseLabel !== '' ? array('cause'=>$ohCauseLabel) : array()),
