...
if($ccdr_date2==""){
	...
	$dClause=" between '".$ccdr_date." 00:00:00' and '".$ccdr_date2." 23:23:59' ";
			...
			if($ccdr_date2==""){
				...
				$ccdr_add=" and to_date like '".$ccdr_date2."%%'";
...
$rowCount++;
		...
		if($page_counter<=19){
			...
			$excel->getActiveSheet()->getStyle("A".$rowCount.":E".($rowCount*1+5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			$excel->getActiveSheet()->getStyle("A".$rowCount.":E".($rowCount*1+5))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);

			$excel->getActiveSheet()->getStyle("I".$rowCount.":M".($rowCount*1+5))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
			$excel->getActiveSheet()->getStyle("I".$rowCount.":M".($rowCount*1+5))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_TOP);
			
			addContent(setRange("A".$rowCount,"E".($rowCount*1+5)),$excel,$summary,"true",$ExWs);
			addContent(setRange("I".$rowCount,"M".($rowCount*1+5)),$excel,$measures,"true",$ExWs);
			$rowCount+=7;
			...
			$rowCount++;
			addContent(setRange("A".$rowCount,"C".$rowCount),$excel,"Chief, Transport Division","true",$ExWs);
			addContent(setRange("E".$rowCount,"G".$rowCount),$excel,"Chief, Engineering Division","true",$ExWs);
			addContent(setRange("I".$rowCount,"J".$rowCount),$excel,"Director for Operations","true",$ExWs);
			addContent(setRange("L".$rowCount,"M".$rowCount),$excel,"General Manager","true",$ExWs);
